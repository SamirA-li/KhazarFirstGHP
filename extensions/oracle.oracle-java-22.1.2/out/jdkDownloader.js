...
exports.fetchJDKDownloadView = exports.extractJDK = exports.JDKDownloader = exports.openJDKSelectionView = exports.fetchDropdownOptions = exports.calculateChecksum = void 0;
const fs = require("fs");
const os = require("os");
...
let customView;
let logger;
const calculateChecksum = (filePath) => __awaiter(void 0, void 0, void 0, function* () {
...
const fetchDropdownOptions = () => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const versions = constants_1.ORACLE_JDK_DOWNLOAD_VERSIONS;
    return { machineArch, osType, versions };
});
exports.fetchDropdownOptions = fetchDropdownOptions;
function openJDKSelectionView(log) {
    return __awaiter(this, void 0, void 0, function* () {
        // Create JDK Downloader view
        customView = vscode.window.createWebviewPanel('jdkDownloader', 'JDK Downloader', vscode.ViewColumn.One, {
            enableScripts: true
        });
        logger = log;
        const { machineArch, osType, versions } = yield (0, exports.fetchDropdownOptions)();
        customView.webview.html = (0, exports.fetchJDKDownloadView)(machineArch, osType, versions);
        customView.webview.onDidReceiveMessage((message) => __awaiter(this, void 0, void 0, function* () {
            const { command, id: jdkType, jdkVersion, jdkOS, jdkArch, installType } = message;
            if (command === 'downloadJDK') {
                const installationPath = yield selectPath(installType);
                if (installationPath) {
                    if (installType === 'manual') {
                        vscode.workspace.getConfiguration('jdk').update('jdkhome', installationPath, true);
                        yield installationCompletion(installType);
                        return;
                    }
...
exports.openJDKSelectionView = openJDKSelectionView;
function JDKDownloader(JDKType, osType, osArchitecture, JDKVersion, installationPath) {
    let downloadUrl = '';
    // Generate download url on the basis of the jdk type chosen
    if (JDKType === 'OpenJDK') {
        if (osType === 'windows') {
            downloadUrl = `${constants_1.OPEN_JDK_VERSION_DOWNLOAD_LINKS[`${JDKVersion}`]}_${osType.toLowerCase()}-${osArchitecture}_bin.zip`;
        }
        else {
            downloadUrl = `${constants_1.OPEN_JDK_VERSION_DOWNLOAD_LINKS[`${JDKVersion}`]}_${osType.toLowerCase()}-${osArchitecture}_bin.tar.gz`;
        }
    }
    else if (JDKType === 'Oracle JDK') {
        if (osType === 'windows') {
            downloadUrl = `${constants_1.ORACLE_JDK_BASE_DOWNLOAD_URL}/${JDKVersion}/latest/jdk-${JDKVersion}_${osType.toLowerCase()}-${osArchitecture}_bin.zip`;
        }
        else {
            downloadUrl = `${constants_1.ORACLE_JDK_BASE_DOWNLOAD_URL}/${JDKVersion}/latest/jdk-${JDKVersion}_${osType.toLowerCase()}-${osArchitecture}_bin.tar.gz`;
        }
    }
    ...
    const request = https.get(downloadUrl, response => {
        ...
        writeStream.on('finish', () => __awaiter(this, void 0, void 0, function* () {
            const checkSumObtained = yield (0, exports.calculateChecksum)(filePath);
            const checkSumExpected = (yield axios_1.default.get(`${downloadUrl}.sha256`)).data;
            if (checkSumExpected === checkSumObtained) {
                ...
                yield extractJDK(filePath, installationPath, JDKVersion, osType, JDKType);
            }
            else {
                (0, extension_1.handleLog)(logger, `Checksums don't match! \n Expected: ${checkSumExpected} \n Obtained: ${checkSumObtained}`);
...
exports.JDKDownloader = JDKDownloader;
function extractJDK(jdkTarballPath, extractionTarget, jdkVersion, osType, jdkType) {
    var e_1, _a;
    return __awaiter(this, void 0, void 0, function* () {
        const downloadedDir = path.join(__dirname, 'jdk_downloads');
        // Remove already present version of a particular JDK from temp dir
        const oldDirs = yield fs.promises.readdir(downloadedDir);
        ...
        try {
            for (var matchingOldDirs_1 = __asyncValues(matchingOldDirs), matchingOldDirs_1_1; matchingOldDirs_1_1 = yield matchingOldDirs_1.next(), !matchingOldDirs_1_1.done;) {
                ...
                yield fs.promises.rmdir(path.join(downloadedDir, oldDirName), { recursive: true });
            }
        }
        ...
        let newDirectoryPath = null;
        child_process.exec(extractCommand, (error) => __awaiter(this, void 0, void 0, function* () {
            if (error) {
                vscode.window.showErrorMessage('Error: ' + error);
            }
            else {
                const dirsPresent = yield fs.promises.readdir(downloadedDir);
                ...
                newDirectoryPath = yield handleJdkPaths(newDirName, extractionTarget, osType);
            ...
            fs.unlink(jdkTarballPath, (err) => __awaiter(this, void 0, void 0, function* () {
                if (err) {
                    vscode.window.showErrorMessage("Error: " + err);
                }
                else {
                    if (tempDirectoryPath && fs.existsSync(tempDirectoryPath)) {
                        yield fs.promises.rmdir(tempDirectoryPath, { recursive: true });
                    }
                    if (newDirectoryPath !== null) {
                        yield installationCompletion("automatic");
                    }
                }
            }));
        }));
...
const handleJdkPaths = (directoryName, parentPath, osType) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (fs.existsSync(directoryPath)) {
        ...
        const selected = yield vscode.window.showInformationMessage(CONFIRMATION_MESSAGE, "Yes", "No");
        if (selected === "Yes") {
...
const selectPath = (installType) => __awaiter(void 0, void 0, void 0, function* () {
    const options = {
    ...
    const selectedFolders = yield vscode.window.showOpenDialog(options);
...
const installationCompletion = (installType) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const selected = yield vscode.window.showInformationMessage(dialogBoxMessage, "Reload now");
    if (selected === "Reload now") {
        yield customView.dispose();
        yield vscode.commands.executeCommand('workbench.action.reloadWindow');
    }
});
const fetchJDKDownloadView = (machineArch, osType, versions) => {
    return `<!DOCTYPE html>
  <head>
      ...
      <div class="jdk-flex-basis">
        ...
        <div class="jdk-version-dropdown">
          <select id="openJDKVersionDropdown">
            ${Object.keys(constants_1.OPEN_JDK_VERSION_DOWNLOAD_LINKS).map((el, index) => {
        if (index === 0) {
            return `<option value=${el} default>JDK ${el}</option>`;
        }
        return `<option value=${el}>JDK ${el}</option>`;
...
exports.fetchJDKDownloadView = fetchJDKDownloadView;
//# sourceMappingURL=jdkDownloader.js.map